@echo off
echo ===============================================================
echo  BeamNG Drive Session Summarizer - Improved Two Server Setup
echo ===============================================================
echo.

REM Store process IDs
set SERVER1_PID=0
set SERVER2_PID=0

REM Create a cleanup function using labels
goto :start
...
if %SERVER1_PID% NEQ 0 (
    echo Terminating Simulation Server (PID: %SERVER1_PID%)...
    taskkill /F /PID %SERVER1_PID% >nul 2>&1
)

if %SERVER2_PID% NEQ 0 (
    echo Terminating Dashboard Server (PID: %SERVER2_PID%)...
    taskkill /F /PID %SERVER2_PID% >nul 2>&1
)

echo.
echo All servers have been terminated. Press any key to exit.
pause >nul
exit /b 0
...
start "Simulation Server - Original UI" cmd /c "echo [Simulation Server] Running on http://localhost:8000 && uvicorn server:app --host 0.0.0.0 --port 8000 --log-level info"

REM Get PID of the first server
for /f "tokens=2" %%a in ('tasklist /fi "windowtitle eq Simulation Server - Original UI*" /fo list ^| find "PID:"') do set SERVER1_PID=%%a
echo Simulation Server started with PID: %SERVER1_PID%
...
start "Dashboard Server - React UI" cmd /c "echo [Dashboard Server] Running on http://localhost:3001 && uvicorn react_server:app --host 0.0.0.0 --port 3001 --log-level debug"

REM Get PID of the second server
for /f "tokens=2" %%a in ('tasklist /fi "windowtitle eq Dashboard Server - React UI*" /fo list ^| find "PID:"') do set SERVER2_PID=%%a
echo Dashboard Server started with PID: %SERVER2_PID%
...
echo Setting up graceful shutdown for Ctrl+C...
if "%~1"=="" (
    REM Only set up the handler the first time
    start /b "" cmd /c "%~f0" triggered
    goto :loop
)

REM Keep this script running until Ctrl+C is pressed
:loop
